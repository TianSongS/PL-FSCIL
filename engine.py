...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    for task_id in range(args.num_tasks): 
        ...
        # Prototype Variable
        # prototype = utils.prototype_update_module(prototype=prototype,model=model,original_model=original_model,
        #                         data_loader=data_loader[task_id]['train'],task_id=task_id,device=device)
        # acc_matrix = utils.prototype_evaluate_till_now_module(prototype=prototype, model=model, original_model=original_model, data_loader=data_loader, device=device, 
        #                             task_id=task_id, class_mask=class_mask, acc_matrix=acc_matrix, args=args)
        
        # Prototype immutable
        prototype = utils.prototype_updata(prototype=prototype,model=model,original_model=original_model,
                                data_loader=data_loader[task_id]['train'],task_id=task_id,device=device)
