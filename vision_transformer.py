...
from prompt import FSCILPrompt, ortho_loss
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.use_dynamic_prototype = use_dynamic_prototype # 使用是否动态原型
        ...
        if use_dynamic_prototype:
            import prototype as pt
            self.head = pt.DynamicPrototypicalCosineClassifier(self.embed_dim, num_classes)
        else:
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting(): 
            ...
            if self.use_d_prompt or self.use_f_prompt: 
                ...
                x = self.blocks(x) 
                res = dict()
        if self.use_d_prompt and self.use_f_prompt:
            res['ortho_loss'] = ortho_loss(self.d_prompt[:,0:1, :, :, :],self.f_prompt.prompt[:, 0:1,task_id ,:, :, :])
            res['ortho_loss'] += ortho_loss(self.d_prompt[:,1:2, :, :, :],self.f_prompt.prompt[:, 1:2,task_id ,:, :, :])
            # print(res['ortho_loss'])
        x = self.norm(x) 
        res['x'] = x
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        res['logits'] = self.head(x) # 输入x:25*768 输出25*200
